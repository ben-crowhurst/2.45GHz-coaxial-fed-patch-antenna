...
if isOctave()
    ...
    fflush(stdout);
    if isunix
        [res, fn_so] = unix('find /usr/lib     -name libhdf5.so           | head -1');
        [res, fn_m]  = unix('find /usr/lib     -name mpi.h      | sort -r | head -1');
        ...
        if length(fn_so)>0 && length(fn_h)>0
            ...
            [mpiinc_dir, mpiinc_fn, ext] = fileparts(fn_m);
            disp(["OpenMPI include path found at: " mpiinc_dir])

            mkoctfile("h5readatt_octave.cc", ["-L" hdf5lib_dir], ["-I" hdf5inc_dir], ["-I" mpiinc_dir], "-L hdf5")
        else
